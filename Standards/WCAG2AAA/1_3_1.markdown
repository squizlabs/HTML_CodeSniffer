...
Incorrect location of the label for a check box. It should be after the check box instead.
...
Incorrect location of a label for a check box. The allowance that a text box label can appear above the label does not apply in reverse (ie. below) for a check box or radio button.
