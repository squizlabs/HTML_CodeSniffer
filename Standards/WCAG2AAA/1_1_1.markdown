...
The image here is presumed to be one that denotes HTML\_CodeSniffer, which does not
...
duplicates the text portion of the link. It would be read as "HTML\_CodeSniffer
HTML\_CodeSniffer" by a screen reader, which is problematic and confusing.
...
Standard example. The alt text should mirror the purpose of the submission action, including any important words on the image (see technique [G94](#tech-g94)).

##### Error example 2

    <input type="image" src="submit.png" />

Error due to no alt attribute provided.

##### Error example 3

    <input type="image" src="submit.png" alt="" />

Error due to empty alt text specified. This would potentially mean the method for submitting the form is hidden from assistive technlogy.

##### Error example 4

    <input type="image" src="submit.png" alt="   " />

Error due to empty alt text specified. Even those there is whitespace, this is the same issue as Example 3.

##### Manual fail example 5

    <input type="image" src="submit.png" alt="big red button" />

Will not fail the automated testing of this technique. However, this should be manually failed, as this description fails item 2 of the testing (it does not describe the purpose of the submission action). It will also fail technique [G94](#tech-g94) as this is not an appropriate short text alternative.
...
The examples below relate to a hypothetical "Subscribe" image, containing a newspaper icon, inviting users to subscribe to a site's mailing list or newsletter.

##### Success example 1

    <img src="subscribe.png" alt="Subscribe to our newsletter" />

Standard example. The alt text should mirror the purpose of the image, including any important words on the image (see technique [G94](#tech-g94)). This may be appropriate if the image states "Subscribe to our newsletter" - or even just "Subscribe", if there is not enough context around the image to determine what you are subscribing to.
...
##### Manual fail example 3

    <img src="subscribe.png" alt="" />

This should not fail this technique alone. However, a warning will be flagged under technique [H67](#tech-h67), which relates to images that should be ignored by assistive technology. In this case, this img element should be manually failed, as it is obvious that this image is **not** purely for decorative purposes.

##### Manual fail example 4

    <img src="subscribe.png" alt="   " />

Should be interpreted the same as example 3. White space should not change how this is interpreted.

##### Manual fail example 5

    <img src="subscribe.png" alt="newspaper icon" />

This would not fail the automated processes of this technique alone. However, a notice will be flagged under technique [G94](#tech-h94), which relates to images that should be ignored by assistive technology. In this case, the img element should be manually failed.
...
HTML\_CodeSniffer will throw an error on this example. If the image contains empty alt text, it should have no title.
