...
- [H45: Using longdesc](#tech-h45)
- [H53: Using the body of the object element](#tech-h53)
...
#### Techniques explicitly not tested

- [H46: Using noembed with embed](#tech-h46)

### <a id="tech-h2">H2: Combining adjacent image and text links for the same resource</a>

[Detailed information about technique H2](http://www.w3.org/TR/2010/NOTE-WCAG20-TECHS-20101014/H2) is available from the W3C. It refers both to combined image-and-text links, and links contained in adjacent cells in a table. Only the first is currently implemented.
...
[Detailed information about technique H24](http://www.w3.org/TR/2010/NOTE-WCAG20-TECHS-20101014/H24) is available from the W3C.
...
[Detailed information about technique H36](http://www.w3.org/TR/2010/NOTE-WCAG20-TECHS-20101014/H36) is available from the W3C.
...
[Detailed information about technique H37](http://www.w3.org/TR/2010/NOTE-WCAG20-TECHS-20101014/H37) is available from the W3C.
...
### <a id="tech-h45">H45: Using longdesc</a>

[Detailed information about technique H45](http://www.w3.org/TR/2010/NOTE-WCAG20-TECHS-20101014/H45) is available from the W3C.

Where images cannot be fully described by a short text alternative, a longdesc attribute can be added to provide a long text alternative. The correct value of a longdesc attribute is a URI which points to a source (whether it be HTML, text)

This is not the only method of providing a long text alternative. Other possible techniques include:

- Adding a link immediately after the non-text content (technique [G73](http://www.w3.org/TR/2010/NOTE-WCAG20-TECHS-20101014/G73)). This was often implemented using a link with text "d", however this is probably not a good link name by itself.
- Providing the long description in the body of the content, and pointing to the location of the text in the short text alternative (technique [G74](http://www.w3.org/TR/2010/NOTE-WCAG20-TECHS-20101014/G74)).

#### Examples

1.  Correct usage with the longdesc attribute pointing to an appropriate URI. A notice
    will be emitted reminding authors to check that the longdesc target is appropriate
    for the attribute.

        <img src="htmlcs_usage.png" alt="Chart of HTML_CodeSniffer usage statistics" longdesc="http://example.com/htmlcs_usage_desc.html" />

#### Warnings

1.  When an image uses the longdesc attribute, but is not an absolute URI. It is
    emitted as a warning because we cannot easily tell whether it is a relative URL
    (eg. just a file name) or junk text.

    Example that should be manually failed:

        <img src="htmlcs_usage.png" alt="Chart of HTML_CodeSniffer usage statistics" longdesc="HTML_CodeSniffer usage increased over the period 2011-......." />

    Example that should be manually passed (so long as the page exists):

        <img src="htmlcs_usage.png" alt="Chart of HTML_CodeSniffer usage statistics" longdesc="htmlcs_usage_desc.html" />

1.  When an image is used without a longdesc attribute. It is fired as a warning to
    alert authors that they should seriously consider whether the short text
    alternative properly describes the image. The message says to use the longdesc
    attribute "or another sufficient technique", such as G73 or G74 above.

    Example (using G74):

        <img src="htmlcs_usage.png" alt="Chart of HTML_CodeSniffer usage statistics - text explanation follows after image." />
        <p>HTML_CodeSniffer usage increased over the period from 2011 to etc...</p>

### <a id="tech-h53">H53: Using the body of the object element</a>

[Detailed information about technique H53](http://www.w3.org/TR/2010/NOTE-WCAG20-TECHS-20101014/H53) is available from the W3C. This technique seems to overlap with [H27: Providing text and non-text alternatives for object](http://www.w3.org/TR/2010/NOTE-WCAG20-TECHS-20101014/H27), which appears to be a half-thought if anything, and at best a duplicate to H53 in terms of process.

This technique provides an accessible text alternative to the object element, which is generally used for non-text content that require external plugins (but is not limited to such).

Multiple object elements can be nested to provide non-text alternatives to the outermost element. However, the innermost object tag element contain a text alternative. Any sort of short text alternative is sufficient, such as alt text from an image.

The text alternative should have the same purpose the contents of the original object - this means it could end up being a **long** text alternative.
...
Note: a notice should be emitted reminding users to check whether the contents are indeed a suitable alternative for the object. In these simplified examples, they are probably not.

1.  Simple text alternative.
...
2.  A text alternative can be block content.
...
3.  Text alternative taken from the alt text of an image.
...
4.  Simple text alternative when nested inside multiple objects (in this case, a Flash file containing a static image as its alternative, which itself as a text alternative).

    Example:

        <object data="htmlcs.swf" type="application/x-shockwave-flash">
            <object data="htmlcs.png" type="image/png">
                Text alternative
            </object>
        </object>
...
1.  No text alternative provided.

    Example:

        <object />

2.  Text alternative taken from the alt text of an image but it is blank.
...
[Detailed information about technique H67](http://www.w3.org/TR/2010/NOTE-WCAG20-TECHS-20101014/H67) is available from the W3C.
...
1.  Non-breaking spaces should not be treated as "empty." This is a rather artificial example and of course would break the rule of alt texts being descriptive.
    ...
    > This currently does not throw anything different to example 1, because it uses "<abbr title="backslash-s">\s</abbr>" as the space character wildcard. The ECMAScript includes <acronym title="non-breaking space">NBSP</acronym> as part of "<abbr title="backslash-s">\s</abbr>", which doesn't fulfil testing item 2.
    >
...
[Detailed information about technique G94](http://www.w3.org/TR/2010/NOTE-WCAG20-TECHS-20101014/G94) is available from the W3C.
...
### <a id="tech-h46">NOT TESTED: H46: Using noembed with embed</a>

[Detailed information about technique H46](http://www.w3.org/TR/2010/NOTE-WCAG20-TECHS-20101014/H46) is available from the W3C.

This technique speaks of marrying the non-standard embed element, used by as a forerunner to the HTML 4.0-standard object element, with a noembed element which provides a text alternative. The noembed element could be placed either inside the embed element, or immediately afterwards.

I am of the belief that this technique should not be used, for the following reasons:

1. Prior to HTML5, the embed element itself was non-standard. Although this alone does not fail Success Criterion 4.1.1 (Parsing) - validation is not a requirement, only well-formedness is - it makes it difficult, if not impossible, to satisfy the sufficient techniques.
2. In HTML5, the embed element is included, however the noembed element is absent. The spec states that the embed element **does not include fallback content**; user agents should use a default plugin instead (even if to say "this content is unsupported"). Therefore, there is no guarantee that the default content will be accessible.

Accessible documents should use the object element instead (see technique [H53](#tech-h53)).
