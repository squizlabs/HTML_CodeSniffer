    ...
    process: function(element, top)
    {
        ...
        if (element === top) {
            ...
            for (var i = 0; i < dblClickEls.length; i++) {
                HTMLCS.addMessage(HTMLCS.WARNING, dblClickEls[i], _global.translation["2_1_1_SCR20.DblClick"], 'SCR20.DblClick');
            }

            var mouseOverEls = HTMLCS.util.getAllElements(top, '*[onmouseover]');
            for (var i = 0; i < mouseOverEls.length; i++) {
                HTMLCS.addMessage(HTMLCS.WARNING, mouseOverEls[i], _global.translation["2_1_1_SCR20.MouseOver"], 'SCR20.MouseOver');
            }

            var mouseOutEls = HTMLCS.util.getAllElements(top, '*[onmouseout]');
            for (var i = 0; i < mouseOutEls.length; i++) {
                HTMLCS.addMessage(HTMLCS.WARNING, mouseOutEls[i],  _global.translation["2_1_1_SCR20.MouseOut"], 'SCR20.MouseOut');
            }

            var mouseMoveEls = HTMLCS.util.getAllElements(top, '*[onmousemove]');
            for (var i = 0; i < mouseMoveEls.length; i++) {
                HTMLCS.addMessage(HTMLCS.WARNING, mouseMoveEls[i], _global.translation["2_1_1_SCR20.MouseMove"], 'SCR20.MouseMove');
            }

            var mouseDownEls = HTMLCS.util.getAllElements(top, '*[onmousedown]');
            for (var i = 0; i < mouseDownEls.length; i++) {
                HTMLCS.addMessage(HTMLCS.WARNING, mouseDownEls[i], _global.translation["2_1_1_SCR20.MouseDown"], 'SCR20.MouseDown');
            }

            var mouseUpEls = HTMLCS.util.getAllElements(top, '*[onmouseup]');
            for (var i = 0; i < mouseUpEls.length; i++) {
                HTMLCS.addMessage(HTMLCS.WARNING, mouseUpEls[i], _global.translation["2_1_1_SCR20.MouseUp"], 'SCR20.MouseUp');
            }
        }
