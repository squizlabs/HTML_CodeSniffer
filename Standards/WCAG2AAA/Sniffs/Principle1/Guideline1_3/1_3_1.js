...
var HTMLCS_WCAG2AAA_Sniffs_Principle1_Guideline1_3_1_3_1 = {
        ...
        if (HTMLCS.util.isAriaHidden(element) === false
            ...
            if (children.length) {
                HTMLCS.addMessage(
                    HTMLCS.ERROR,
                    element,
                    HTMLCS.getTranslation("1_3_1_F92,ARIA4"),
                    'F92,ARIA4'
                );
            }
        }
            ...
            if ((labels[i].getAttribute('for') !== null) && (labels[i].getAttribute('for') !== '')) {
                ...
                if ((this._labelNames[labelFor]) && (this._labelNames[labelFor] !== null)) {
                    ...
                    if (refNode === null) {
                        ...
                        var msg   = HTMLCS.getTranslation("1_3_1_H44.NonExistent");
                        var code  = 'H44.NonExistent';
                        if ((HTMLCS.isFullDoc(top) === true) || (top.nodeName.toLowerCase() === 'body')) {
                            level = HTMLCS.WARNING;
                            msg   = HTMLCS.getTranslation("1_3_1_H44.NonExistentFragment");
                        ...
                        if ('input|select|textarea|button|keygen|meter|output|progress'.indexOf(nodeName) === -1) {
                            HTMLCS.addMessage(HTMLCS.WARNING, labels[i], HTMLCS.getTranslation("1_3_1_H44.NotFormControl"), 'H44.NotFormControl');
                        }
                    }
                }
            }
        ...
        if (title !== null) {
            if ((/^\s*$/.test(title) === true) && (needsLabel === true)) {
                HTMLCS.addMessage(
                    HTMLCS.WARNING,
                    element,
                    HTMLCS.getTranslation("1_3_1_H65"),
                    'H65'
                );
            } else {
        ...
        if (element.hasAttribute('aria-labelledby') === true) {
            if (HTMLCS.util.hasValidAriaLabel(element) === false) {
                HTMLCS.addMessage(
                    HTMLCS.WARNING,
                    element,
                    HTMLCS.getTranslation("1_3_1_ARIA16,ARIA9").replace(/\{\{id\}\}/g, element.getAttribute('aria-labelledby')),
                    'ARIA16,ARIA9'
                );
            } else {
            ...
            if ((hasLabel !== false) && (needsLabel === false)) {
                ...
                if (inputType === 'hidden') {
                    HTMLCS.addMessage(
                        HTMLCS.WARNING,
                        element,
                        HTMLCS.getTranslation("1_3_1_F68.Hidden"),
                        'F68.Hidden'
                    );
                ...
                HTMLCS.addMessage(
                    HTMLCS.ERROR,
                    element,
                    HTMLCS.getTranslation("1_3_1_F68"),
                    'F68'
                );
            }//end if
        }
    ...
    testPresentationMarkup: function(top)
    {
        ...
        if (doctype && (doctype === 'html5' || doctype === 'xhtml5')) {
            ...
            for (var i = 0; i < tags.length; i++) {
                ...
                HTMLCS.addMessage(HTMLCS.ERROR, tags[i],  HTMLCS.getTranslation("1_3_1_H49."), msgCode);
            }
            ...
            for (var i = 0; i < tags.length; i++) {
                var msgCode = 'H49.' + tags[i].nodeName.substr(0, 1).toUpperCase() + tags[i].nodeName.substr(1).toLowerCase();
                HTMLCS.addMessage(HTMLCS.WARNING, tags[i], HTMLCS.getTranslation("1_3_1_H49.Semantic"), msgCode);
            }
            ...
            for (var i = 0; i < tags.length; i++) {
                var msgCode = 'H49.AlignAttr';
                HTMLCS.addMessage(HTMLCS.WARNING, tags[i], HTMLCS.getTranslation("1_3_1_H49.AlignAttr.Semantic"), msgCode);
            }
        }
    },
        ...
        if (tag === 'p' || tag === 'div') {
            ...
            if ((children.length === 1) && (children[0].nodeType === 1)) {
                ...
                if (/^(strong|em|b|i|u)$/.test(childTag) === true) {
                    HTMLCS.addMessage(HTMLCS.WARNING, element, HTMLCS.getTranslation("1_3_1_H42"), 'H42');
                }
            }
        }
    },
    ...
    testTableHeaders: function(table)
    {
        ...
        for (var i = 0; i < scopeAttr.invalid.length; i++) {
            HTMLCS.addMessage(HTMLCS.ERROR, scopeAttr.invalid[i],  HTMLCS.getTranslation("1_3_1_H63.3"), 'H63.3');
        }
        ...
        for (var i = 0; i < scopeAttr.obsoleteTd.length; i++) {
            HTMLCS.addMessage(HTMLCS.WARNING, scopeAttr.obsoleteTd[i], HTMLCS.getTranslation("1_3_1_H63.2"), 'H63.2');
        }
        ...
        if (headersAttr.allowScope === true) {
            ...
            if (scopeAttr.used === true) {
                HTMLCS.addMessage(HTMLCS.WARNING, table, HTMLCS.getTranslation("1_3_1_H43.ScopeAmbiguous"), 'H43.ScopeAmbiguous');
                scopeAttr = null;
            }
        }//end if

        if (headersAttr.isMultiLevelHeadersTable) {
            HTMLCS.addMessage(HTMLCS.NOTICE, table, HTMLCS.getTranslation("1_3_1_H43.IncorrectAttrNotice"), 'H43.IncorrectAttr');
        } else {
            // Incorrect usage of headers - error; emit always.
            for (var i = 0; i < headersAttr.wrongHeaders.length; i++) {
                HTMLCS.addMessage(HTMLCS.ERROR, headersAttr.wrongHeaders[i].element, HTMLCS.getTranslation("1_3_1_H43.IncorrectAttr").replace(/\{\{expected\}\}/g, headersAttr.wrongHeaders[i].expected).replace(/\{\{actual\}\}/g, headersAttr.wrongHeaders[i].actual), 'H43.IncorrectAttr');
            }
        }
        ...
        if ((headersAttr.required === true) && (headersAttr.allowScope === false)) {
            if (headersAttr.used === false) {
                ...
                HTMLCS.addMessage(HTMLCS.ERROR, table,  HTMLCS.getTranslation("1_3_1_H43.HeadersRequired"), 'H43.HeadersRequired');
            } else {
                // Missing TH IDs - error; emit at this stage only if headers are compulsory.
                if (headersAttr.missingThId.length > 0) {
                    HTMLCS.addMessage(HTMLCS.ERROR, table, HTMLCS.getTranslation("1_3_1_H43.MissingHeaderIds"), 'H43.MissingHeaderIds');
                }

                // Missing TD headers attributes - error; emit at this stage only if headers are compulsory.
                if (headersAttr.missingTd.length > 0) {
                    HTMLCS.addMessage(HTMLCS.ERROR, table, HTMLCS.getTranslation("1_3_1_H43.MissingHeadersAttrs"), 'H43.MissingHeadersAttrs');
                }
            }//end if
        }//end if
        ...
        if ((headersAttr.required === true) && (headersAttr.allowScope === true) && (headersAttr.correct === false) && (scopeAttr.correct === false)) {
            if ((scopeAttr.used === false) && (headersAttr.used === false)) {
                // Nothing used at all.
                HTMLCS.addMessage(HTMLCS.ERROR, table, HTMLCS.getTranslation("1_3_1_H43,H63"), 'H43,H63');
                ...
                HTMLCS.addMessage(HTMLCS.ERROR, table, HTMLCS.getTranslation("1_3_1_H63.1"), 'H63.1');
    ...
    testTableCaptionSummary: function(table) {
        ...
        if (doctype && doctype.indexOf('html5') === -1) {
            ...
            if (summary !== '') {
                if (HTMLCS.util.isLayoutTable(table) === true) {
                    HTMLCS.addMessage(HTMLCS.ERROR, table, HTMLCS.getTranslation("1_3_1_H73.3.LayoutTable"), 'H73.3.LayoutTable');
                } else {
                    if (caption === summary) {
                        HTMLCS.addMessage(HTMLCS.ERROR, table, HTMLCS.getTranslation("1_3_1_H39,H73.4"), 'H39,H73.4');
                    }

                    HTMLCS.addMessage(HTMLCS.NOTICE, table,  HTMLCS.getTranslation("1_3_1_H73.3.Check"), 'H73.3.Check');
                }
            } else {
                if (HTMLCS.util.isLayoutTable(table) === false) {
                    HTMLCS.addMessage(HTMLCS.WARNING, table, HTMLCS.getTranslation("1_3_1_H73.3.NoSummary"), 'H73.3.NoSummary');
                }
            }//end if
        }//end if
        ...
        if (caption !== '') {
            if (HTMLCS.util.isLayoutTable(table) === true) {
                HTMLCS.addMessage(HTMLCS.ERROR, table, HTMLCS.getTranslation("1_3_1_H39.3.LayoutTable"), 'H39.3.LayoutTable');
            } else {
                HTMLCS.addMessage(HTMLCS.NOTICE, table, HTMLCS.getTranslation("1_3_1_H39.3.Check"), 'H39.3.Check');
            }
        } else {
            if (HTMLCS.util.isLayoutTable(table) === false) {
                HTMLCS.addMessage(HTMLCS.WARNING, table, HTMLCS.getTranslation("1_3_1_H39.3.NoCaption"), 'H39.3.NoCaption');
            }
        }//end if
    },
    ...
    testFieldsetLegend: function(fieldset) {
        ...
        if ((legend === null) || (legend.parentNode !== fieldset)) {
            HTMLCS.addMessage(HTMLCS.ERROR, fieldset, HTMLCS.getTranslation("1_3_1_H71.NoLegend"), 'H71.NoLegend');
        }
    },
    ...
    testOptgroup: function(select) {
        ...
        if (optgroup === null) {
            ...
            HTMLCS.addMessage(HTMLCS.WARNING, select, HTMLCS.getTranslation("1_3_1_H85.2"), 'H85.2');
        }
    },
    ...
    testRequiredFieldsets: function(form) {
        ...
        for (var i = 0; i < optionInputs.length; i++) {
            ...
            if (usedNames[optionName] === undefined) {
                ...
                HTMLCS.addMessage(HTMLCS.WARNING, form, HTMLCS.getTranslation("1_3_1_H71.SameName"), 'H71.SameName');
                break;
            }//end if
        }//end for
    },
        ...
        if (firstBreak !== null) {
            ...
            for (var i = 0; i < items.length; i++) {
                if (/^[-*]\s+/.test(items[0]) === true) {
                    // Test for "- " or "* " cases.
                    HTMLCS.addMessage(HTMLCS.WARNING, element,  HTMLCS.getTranslation("1_3_1_H48.1"), 'H48.1');
                    break;
                } if (/^\d+[:/\-.]?\s+/.test(items[0]) === true) {
                    // Test for "1 " cases (or "1. ", "1: ", "1- ").
                    HTMLCS.addMessage(HTMLCS.WARNING, element, HTMLCS.getTranslation("1_3_1_H48.2"), 'H48.2');
                    break;
                }
            }//end for
        }//end if
        ...
        for (var i = 0; i < headings.length; i++) {
            ...
            if (headingNum - lastHeading > 1) {
                if (lastHeading === 0) {
                    ...
                    HTMLCS.addMessage(level, headings[i], HTMLCS.getTranslation("1_3_1_G141_a").replace(/\{\{headingNum\}\}/g, headingNum), 'G141');
                }

                HTMLCS.addMessage(level, headings[i], HTMLCS.getTranslation("1_3_1_G141_b").replace(/\{\{headingNum\}\}/g, headingNum).replace(/\{\{properHeadingNum\}\}/g, lastHeading + 1), 'G141');
            }
    ...
    testEmptyHeading: function(element) {
        ...
        if (/^\s*$/.test(text) === true) {
            HTMLCS.addMessage(HTMLCS.ERROR, element, HTMLCS.getTranslation("1_3_1_H42.2"), 'H42.2');
        }
    },
    ...
    testUnstructuredNavLinks: function(element)
    {
        ...
        if (linksLength > 1) {
            ...
            if (parent === null) {
                HTMLCS.addMessage(HTMLCS.WARNING, element, HTMLCS.getTranslation("1_3_1_H48"), 'H48');
            }
        }//end if
    },
    ...
    testGeneralTable: function(table) {
        if (HTMLCS.util.isLayoutTable(table) === true) {
            HTMLCS.addMessage(HTMLCS.NOTICE, table, HTMLCS.getTranslation("1_3_1_LayoutTable"), 'LayoutTable');
        } else {
            HTMLCS.addMessage(HTMLCS.NOTICE, table, HTMLCS.getTranslation("1_3_1_DataTable"), 'DataTable');
        }
    }
};
