            ...
            for (var i = 0; i < failures.length; i++) {
                ...
                if (recommend) {
                    if (recommend.fore.from !== recommend.fore.to) {
                        recommendText.push(_global.translation["1_4_3_G18_or_G145.Fail.Recomendation.Text"] + ' ' + recommend.fore.to);
                    }
                    if (recommend.back.from !== recommend.back.to) {
                        recommendText.push(_global.translation["1_4_3_G18_or_G145.Fail.Recomendation.Background"] + ' ' + recommend.back.to);
                    }
                }//end if

                if (recommendText.length > 0) {
                    recommendText = ' ' + _global.translation["1_4_3_G18_or_G145.Fail.Recomendation"] + ' ' + recommendText.join(', ') + '.';
                }

                if (isAbsolute === true) {
                    code += '.Abs';
                    HTMLCS.addMessage(HTMLCS.WARNING, element, _global.translation["1_4_3_G18_or_G145.Abs"].replace(/\{\{required\}\}/g, required), code);
                } else if (hasBgImg === true) {
                    code += '.BgImage';
                    HTMLCS.addMessage(HTMLCS.WARNING, element,  _global.translation["1_4_3_G18_or_G145.BgImage"].replace(/\{\{required\}\}/g, required), code);
                } else {
                    code += '.Fail';
                    HTMLCS.addMessage(HTMLCS.ERROR, element, _global.translation["1_4_3_G18_or_G145.Fail"].replace(/\{\{required\}\}/g, required).replace(/\{\{value\}\}/g, value) + recommendText, code);
                }//end if
            }//end for
        }//end if
    }
